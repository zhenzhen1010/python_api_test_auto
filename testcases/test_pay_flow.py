...
# 数据库断言
from tools.handle_db_assert import db_assert_fun

# 第二步 调用excel方法，读取文件数据
# case_all = read_excel(excel_path, "支付业务流")
# case_all = read_excel(excel_path, "支付业务流-数据库断言")
case_all = read_yaml(yaml_path/"pay_data.yaml")
# print(case_all)
# 创建Requests请求实例
# requests = Requests()
...
def test_order_flow(case):
    ...
    expected = case["assertion"] # 期望结果
    assert_db = case["assert_db"] # 数据库断言
